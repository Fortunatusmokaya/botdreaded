...
export default class Command extends BaseCommand {
    ...
    run = async (M: ISimplifiedMessage): Promise<void> => {
        ...
        if (this.client.config.mods?.includes(M.sender.jid)) {
            ...
            if (status === 401) return void M.reply('Cannot join group. Maybe, I was removed from there before or link was reset or group full')
            if (groups.includes(gid)) return void M.reply('Already there')
            return void M.reply(`Successfully Joined ${(await this.client.fetchGroupMetadataFromWA(gid)).subject}`)
        }
    }
}
